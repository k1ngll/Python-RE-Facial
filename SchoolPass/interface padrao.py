...
import sqlite3
import datetime
import sys # Import sys for BASE_DIR logic in executable

# --- DATABASE CONFIGURATION ---
# Get the directory of the current script (where db.sqlite3 is also located)
if getattr(sys, 'frozen', False):
    # If the application is frozen (it's an executable)
...
def busca_sala(curso_id):
    ...
    if curso_id is None:
        ...
        return "Funcionario" # Alterado para 'Funcionario' conforme sua necessidade
...
# --- REMOVED: Fullscreen window settings ---
# Define o nome da janela (ainda é necessário)
window_name = "Reconhecimento Facial - Pressione 'q' para sair"
cv2.namedWindow(window_name, cv2.WINDOW_NORMAL) # Criar a janela normalmente redimensionável
# As linhas cv2.setWindowProperty para FULLSCREEN e AUTOSIZE foram removidas/comentadas aqui.
# --- END REMOVED ---
